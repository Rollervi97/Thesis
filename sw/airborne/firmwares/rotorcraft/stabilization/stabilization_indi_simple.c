...
#include "modules/system_identification/sys_id_wave.h"
...
//  indi.du.r = chirp_in->current_value * scaling factor
  ...
  if (stabilization_cmd[COMMAND_THRUST] < 300 && !in_flight) {
    ...
    if ((sys_id_doublet_running()) || (sys_id_chirp_running()) || (sys_id_wave_running()))
    {
      printf("Either chirp, doublet or wave is running \n");
      indi.u_in.r = 0.0;
    } 
    else 
    {
      printf("Normal INDI is running \n");
